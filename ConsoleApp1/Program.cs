...
{
    private const string PATH = "C:\\Users\\Кирилл\\Desktop\\1\\28-09.pcap";
    ...
    static void Main(string[] args)
    {
        ...
        {
            var voice = item.Value.GetVoice();
            if (voice.Length == 0)
                continue;

            var decodeVoice = ALawDecoder.Decode(voice);

            var fileName = item.Key.ToString().Replace("-", "");
